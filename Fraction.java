...
public class Fraction {
    ...
    * @param another Fraction to add this Fraction with
    * @return new Fraction as a sum of the addend and this Fraction
    *
    */
     ...
     /**
      * Substracts this Fraction with another Fraction. 
      * @param another Fraction to substract this Fraction with
   ...
   /**
   * Checks if the syntax of the String is a valid fraction and then makes a Fraction out of the String
   *
   * @param Fraction as a String
   * @return The String as á Fraction
   */
   
   public Fraction parse_String_to_Fraction(String input) {
	      try{
	   	   input.matches("-?\\d+/[1-9]\\d*");
	      }catch(Exception e) {
	    	throw new NumberFormatException("Parsing is not possible");
	       }
	      
	      String[] splittedString = input.split("/");
	      int num = Integer.parseInt(splittedString[0]);
		  int den = Integer.parseInt(splittedString[1]);
		  return new Fraction(num, den);;
		   
	}
   /*
   public String parseFraction(String one, String operator,String two) {
	   Fraction first = parse_String_to_Fraction(one);
	   Fraction second = parse_String_to_Fraction(two);
	   String output = first.toString() + operator + second.toString();
	   return output;
   }
